...
flowchart TD
    subgraph Layers["Testing Pyramid"]
        E2E["End-to-End Multi-Session Simulation (tests/e2e)<br/>• Channel -> Workspace -> Worker -> Cron Pipeline<br/>• Full Turn Correlation & State Machines<br/>• Anomaly Detection & Recovery"]
        Component["Component & Contract Tests (tests/unit)<br/>• amux.py (HTTPX Client, Kanban, Sessions)<br/>• hindsight.py (Reflect, Recall, Retain, Models)<br/>• diagnostics.py (Traceback & Failure Context)<br/>• bin/membank-ctl (CLI Flags & YAML Parser)"]
...
2. **End-to-End (E2E) Multi-Session Simulation:** Tests complete multi-agent workflows across simulated `mypai-channel`, `mypai-workspace`, `amux-task-worker`, and `mypai-cron` sessions, verifying end-to-end turn routing and Kanban state transitions.
...
| **Multi-Session E2E** | Multi-Agent Mesh | `tests/e2e/test_multi_session_flow.py` | • Ingress -> Channel -> Workspace -> Worker -> Done pipeline<br/>• Cron sweep -> Probing -> Anomaly Detection -> Kanban Alert card |
...
| :--- | :--- | :--- |
| `make buildenv` | Provision `.venv` and install `mypai_eval_runtime` in editable mode with test tools | `uv venv`, `uv pip install -e ".[test]"` |
...
| `make coverage` | Generate line-level test coverage report and XML artifact | `pytest --cov=mypai_eval_runtime --cov-report=term-missing` |
...
sequenceDiagram
    ...
    participant Workspace as mypai-workspace
    ...
    Channel->>Workspace: amux.send_message("mypai-workspace", correlation_id="req-1001")
    
    Workspace->>Board: amux.create_card("Refactor DB", lane="Todo")
    Workspace->>Worker: amux.spawn_task_worker("task-worker-db-1", prompt="...")
    Workspace->>Board: amux.update_card(lane="Doing")
    
    Worker->>Worker: Executes refactoring & runs pytest
    Worker->>Workspace: amux.send_message("mypai-workspace", correlation_id="report-1001")
    
    Workspace->>Memory: hindsight.retain("Increased asyncpg max_size to 100")
    Workspace->>Board: amux.update_card(lane="Done")
    Workspace->>Channel: amux.send_message("mypai-channel", correlation_id="req-1001")
...
3. **Automated Kanban Alerting:** Verifies that cron automatically posts an alert card to the `Todo` lane with diagnostic context, notifying the workspace without crashing the scheduler.
...
### Unit Test Pattern
When adding a new feature or API method to `mypai_eval_runtime`:
