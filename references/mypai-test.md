...
| :--- | :--- | :--- | :--- |
| **Vector Memory Client** | `mypai_runtime.hindsight` | `tests/unit/test_hindsight_client.py` | • Cross-bank REST operations across specified `bank_id` targets (`POST /v1/banks/{id}/reflect`, `GET /v1/banks/{id}/recall`, `POST /v1/banks/{id}/retain`, `GET /v1/banks/{id}/mental-models`, `POST /v1/banks/{id}/consolidate`) (for default session banks, in-process OMP loopback tools `tool.reflect()`, `tool.recall()`, `tool.retain()` are used) |
